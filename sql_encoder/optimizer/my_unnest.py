...
def decorrelate(select, parent_select, external_columns, next_alias_name):
    ...
    original_parent_predicate = parent_predicate.copy()
    ...
    if parent_predicate.find(exp.ArrayAny, exp.ArrayContains, exp.ArrayAll) is None and select.find(exp.ArrayAgg) is None:
        parent_select.join(
        ...
        parent_predicate = parent_predicate.replace(original_parent_predicate)
        if isinstance(parent_predicate, exp.Exists):
            parent_predicate.this.args['expressions'] = [sqlglot.parse_one("*")]
