...
def process_file(file_path, node_cnt_threshold):
    filter_sqls = []
    with open(file_path, "r", errors="replace", encoding='utf-8') as sqls_file:
...
def filter_queries(input_data_path, dataset_name, training_size, node_cnt_threshold, split_size):
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=os.cpu_count() // 2) as executor:
        results = executor.map(process_file, file_paths, [node_cnt_threshold for _ in range(len(file_paths))])
    ...
    for index, queries in enumerate(training_queries_split):
        with open(f"{training_queries_path}/queries_split_{index}.txt", "w", encoding='utf-8') as training_queries_file:
            for query in queries:
                training_queries_file.write(query)
    for index, queries in enumerate(evaluation_queries_split):
        with open(f"{evaluation_queries_path}/queries_split_{index}.txt", "w", encoding='utf-8') as evaluation_queries_file:
            for query in queries:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--training_set_size', type=int, default=300000, help="The count of training SQL queries.")
