...
from modules.recommender import recommend_config
from modules.history_tuner import update_history
...
def run_task_and_update(task_id, config, logger, update=True):
    app_idx = run_task(task_id, config)  # 马上开始跑，输出的是'app_id_1/app_id_2'这种形式
    if app_idx == "":
...
def recommend_config_for_new_task(task_id, num_epochs=1):
    ...
    while True:
        ...
        logger.info(small_gap)
        config = update_history(task_id, epochs, logger)
        logger.info(small_gap)
        if config is None:
            return
...
def recommend_config_for_new_task_without_history(task_id, random_sample_epochs=10, model_sample_epochs=30):
    ...
    existing_data = existing_data[existing_data['task_id'] == task_id].head(random_sample_epochs)  # TODO: 随机？头几个？时间最小的几个？
    ...
    while True:
        logger.info(small_gap)
        config = update_history(task_id, epochs + 1, logger)
        logger.info(small_gap)
        if config is None:
            return
...
def update_history_task(update_task_id, num_epochs):
    ...
    while True:
        logger.info(small_gap)
        config = update_history(update_task_id, epochs + 1, logger)
        logger.info(small_gap)
        if config is None:
            return
...
def recommend_config_for_new_task_with_exp3(task_id, num_epochs=1):
    init_tuning_data(all_history_data_file_path)
